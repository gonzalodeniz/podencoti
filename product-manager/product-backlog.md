...
- `PB-011` deja de estar en `nuevo`: `qa-teams` valido la entrega en la issue #11 el 2026-03-27 sobre la rama `developer-teams/pb-011-consolidacion-atom`, pero sigue pendiente la integracion en `main` y el cierre administrativo posterior.
...
1. Integrar en `main` y cerrar administrativamente `PB-011`
2. PB-012
3. PB-010
4. PB-005
5. PB-008
...
- `PB-011` ya fue validado por `qa-teams`, pero mientras `developer-teams` no deje evidencia de fusion en `main` y borrado de rama, `product-manager` no debe tratarlo como `cerrado` ni impulsar el arranque efectivo de `PB-012` como si la dependencia estuviera completada.
...
- Existe una dependencia operativa abierta mientras `PB-011` siga validado pero no integrado en `main`: `PB-012` no debe tratarse como siguiente entrega inmediata sin cerrar antes esa integracion base.
